insertDiagonallyUp(char wordsList[7][10][12], char wordSearch[25][25], int storage[10][3], int currentWord, 
                   int wordLength, int category)
{
	...
	while (pass == 0)
	{
		...
		height = (rand() % (25 - wordLength)) + wordLength;
		width = rand() % (25 - wordLength);
	
		//checking for overlap
		for (x = 0; x < wordLength; x++)
		{
			if (wordSearch[(height - x)][(width + x)] != '.')
				overlap++;
		} 
		...
		if (overlap == 0)
		{
			storage[currentWord][0] = 2;
			storage[currentWord][1] = height;
			storage[currentWord][2] = width;
			
			for (x = 0; x < wordLength; x++)
				wordSearch[(height - x)][(width + x)] = wordsList[category][currentWord][x];
			currentWord++;
			pass++;
		}
    }
